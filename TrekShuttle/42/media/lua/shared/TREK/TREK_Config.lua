...
-- The mod's map folder (common/media/maps/TrekShuttle): mapped cells round
-- the cabin and the Adirondack, so the world generator never grows grass,
-- trees or zombies there; their ground is a starfield. Generated by
-- tools/gen_void_map.py. **It has to be under common/**: the engine looks for
-- a mod's maps there and skips the mod entirely when the folder is missing,
-- never reaching 42/media/maps (MapGroups.createGroups). Single player adds it
-- by itself from there; a dedicated server lists it in its Map= setting.
...
C.PhobiaPanic   = 40
C.PhobiaUnhappy = 5
-- Turbolift phobia (JEFFERIES.md), per ride: instant dread, worse than a
-- beam, because the tubes were right there.
C.LiftPhobiaStress  = 0.35
C.LiftPhobiaPanic   = 60
C.LiftPhobiaUnhappy = 10
